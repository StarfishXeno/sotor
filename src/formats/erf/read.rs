...
impl<'a> Reader<'a> {
    fn new(c: Cursor<&'a [u8]>, h: Header) -> Self {
        Self { c, h }
    }
    ...
    fn read_header(c: &mut Cursor<&[u8]>) -> SResult<Header> {
        ...
        let [loc_string_count, _, entry_count, loc_string_offset, keys_offset, resources_offset, _, _, description_str_ref] =
            take::<[u32; 9]>(c)
                .ok_or("couldn't read header data")?
                .to_usize_vec()
                .try_into()
                .unwrap();
    ...
    fn read_loc_strings(&mut self) -> SResult<Vec<LocString>> {
        ...
        while count < target_count {
            let [id, len] = take::<[u32; 2]>(&mut self.c)
                .ok_or_else(|| format!("Couldn't read LocStr head {count}"))?;

            let content = take_string(&mut self.c, len as usize)
    ...
    fn read_key_list(&mut self) -> SResult<Vec<KeyRead>> {
        ...
        let key_bytes = take_slice::<[u8; KEY_SIZE_BYTES]>(&mut self.c, self.h.entry_count)
            .ok_or("couldn't read key list")?;

        for (idx, bytes) in key_bytes.iter().enumerate() {
            let c = &mut Cursor::new(bytes.as_ref());
            let name = take_string(c, KEY_NAME_LEN).unwrap();
    ...
    fn read_resources(&mut self) -> SResult<Vec<ResourceRead>> {
        ...
        let resource_dwords = take_slice::<[u32; 2]>(&mut self.c, self.h.entry_count)
            .ok_or("couldn't read resources")?;
    ...
    fn transform(
        ...
        for (idx, key) in keys.into_iter().enumerate() {
            ...
            let resource = Resource {
                id: key.id,
                content: take_slice::<u8>(&mut self.c, res.size)
                    .ok_or_else(|| {
